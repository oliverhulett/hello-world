...
public class DatabaseTest {
	...
	public void currentSchemaVersionDoesMatchMappings() throws SQLException {
		...
		List<SchemaUpdateScript> scripts = sessionFactory
	...
	public void schemaMigrationIsValid() throws SQLException {
		if (DatabaseTest.previousVersion == null) {
			return;
		}

		String previousSchema = String.format("sql/schema_v%s.sql",
				DatabaseTest.previousVersion);
		String migrationScript = String.format("sql/schema_v%s_to_v%s.sql",
				DatabaseTest.previousVersion, DatabaseTest.version);
		Resource previousSchemaResource = new ClassPathResource(previousSchema);
		Resource migrationScriptResource = new ClassPathResource(
				migrationScript);
		ScriptUtils.executeSqlScript(dataSource.getConnection(),
				previousSchemaResource);
		ScriptUtils.executeSqlScript(dataSource.getConnection(),
				migrationScriptResource);
